...
public class Game {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Bienvenue dans le jeu de Pocket Imperium !");
        System.out.println("A quelle version du jeux souhaitez-vous jouer ?");
        ...
        try (Scanner scanner = new Scanner(System.in)) {
            ...
            while (choixConsGUI < 1 || choixConsGUI > 2) {
                System.out.println("Combien de joueurs réels vont jouer ? (1 à 2)");
                try {
                    choixConsGUI = scanner.nextInt();
                    if (choixConsGUI < 0 || choixConsGUI > 4) {
                        System.out.println("Erreur : Veuillez entrer un nombre compris entre 1 et 2.");
                    }
        ...
        if (choixConsGUI == 1){
            ...
            while (nombreJoueurs < 0 || nombreJoueurs > 4) {
                ...
                try {
                    nombreJoueurs = Integer.parseInt(console.getInput());
                    if (nombreJoueurs < 0 || nombreJoueurs > 4) {
            ...
            for (int i = 0; i < nombreJoueurs; i++) {
                ...
                String nomJoueur = console.getInput();
                game.addPlayer(new Player(nomJoueur, 0));
            }
            ...
            for (int i = 0; i < 9; i++) {
                ...
                List<List<Player>> SensPlayer = game.sensplayer(game); // Occuper des cartes et calculer l'ordre des joueurs

                for (int j = 0; j < SensPlayer.size() - 1; j++) {
                    if (j == 0) {
                        console.println("C'est donc au tour de " + SensPlayer.get(0).get(0).getPlayerName());
                    } else {
                        console.println(" puis de " + SensPlayer.get(0).get(j).getPlayerName());
                    }
                }
