    ...
    public void invade(Player player, HashMap<String, ArrayList<SectorCard>> plateau, ConsoleGUI console) {
        ...
        for (int i = 0; i < maxInvasions; i++) {
            ...
            for (Hex adj : hexCible.rexAdjacent(plateau)) {
                ...
                for (Hex adj2 : adj.rexAdjacent(plateau)) {
                    for (Ship ship : adj2.getOccupation()) {
                        if (ship.getPlayerName().equals(player)) {
                            adjacentPlayerHexes.add(adj2);
                        }
                    }
                }
            }
            ...
            if (sommedef < shipsToUse){
                ...
                while (shipsToUse != 0){
                    for (Ship ship : hexCible.getOccupation()){
                        ...
                        console.removeShipsFromHex(hexCible.getId(), ship.getPlayerName().getColor());
                        int nvNbr = ship.getNbrShipy();
                        for (int j = 0; j < nvNbr; j++) {
                            console.placeShipInHex(hexCible.getId(), ship.getPlayerName().getColor());
                        }
                        shipsToUse -= 1;
                    }
                }
